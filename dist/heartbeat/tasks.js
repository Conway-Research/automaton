...
export const BUILTIN_TASKS = {
    ...
    self_check: async (_ctx, taskCtx) => {
        ...
        try {
            ...
            if (health8080.ok) {
                const data = (await health8080.json());
                passed.push("8080:/health");
                // Version consistency check
                if (data.version && data.version !== CODE_VERSION) {
                    issues.push(`8080 version mismatch: code=${CODE_VERSION}, running=${data.version}`);
                }
            }
            else {
        ...
        if (issues.length > 0) {
            logger.warn(`self_check found ${issues.length} issue(s): ${issues.join("; ")}`);
            return {
                shouldWake: true,
    ...
    find_customers: async (_ctx, taskCtx) => {
        try {
            const { discoverAgents } = await import("../registry/discovery.js");
            const { filterPotentialCustomers } = await import("../registry/filters.js");
            const { sendBulkOffers, recordOutreachResult, getOutreachStats } = await import("../registry/outreach.js");
            const MY_AGENT_ID = Number(taskCtx.config.agentId || 18893);
            // Get already-contacted agents
            const contacted = new Set(JSON.parse(taskCtx.db.getKV("find_customers_contacted") || "[]"));
            // Scan up to 50 agents
            const agents = await discoverAgents(50, "mainnet", undefined, taskCtx.db.raw);
            // Filter out self
            const filteredAgents = agents.filter((agent) => {
                const agentId = BigInt(agent.agentId || 0);
                if (agentId === BigInt(MY_AGENT_ID))
                    return false;
                return true;
            });
            // Phase 2: Apply keyword filtering for financial agents
            const potentialCustomers = filterPotentialCustomers(filteredAgents, {
                excludeIds: contacted,
                limit: 20,
                minScore: 20,
            });
            ...
            if (agents.length > 0) {
                const maxId = Math.max(...agents.map((a) => parseInt(String(a.agentId), 10)));
            ...
            if (potentialCustomers.length > 0 && taskCtx.social) {
                outreachResults = await sendBulkOffers(taskCtx.social, potentialCustomers, { myAgentId: MY_AGENT_ID }, 5);
                // Record results
            ...
            // Log discovery
            taskCtx.db.setKV("last_find_customers", JSON.stringify({
            ...
            logger.info(`find_customers: scanned ${agents.length}, filtered ${potentialCustomers.length} leads, sent ${outreachResults.filter((r) => r.success).length} offers`);
            ...
            logger.error("find_customers failed", error instanceof Error ? error : undefined);
            return { shouldWake: false };
        }
    },
};
