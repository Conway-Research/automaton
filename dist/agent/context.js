 ...
 */
export function truncateToolResult(result, maxSize = MAX_TOOL_RESULT_SIZE) {
 ...
 */
export function buildContextMessages(systemPrompt, recentTurns, pendingInput, options) {
    ...
    if (analysisWindow.length >= 3) {
        ...
        if (repeatedTools.length > 0) {
            messages.push({
                role: "user",
                content: `[system] WARNING: You have been calling ${repeatedTools.join(", ")} repeatedly in recent turns. ` +
