 ...
 */
function createShellInjectionRule() {
    return {
        ...
        evaluate(request) {
            ...
            for (const field of fields) {
                ...
                if (SHELL_METACHAR_RE.test(value)) {
                    return deny("command.shell_injection", "SHELL_INJECTION_DETECTED", `Shell metacharacter detected in ${request.tool.name}.${field}: "${value.slice(0, 50)}"`);
                }
            }
            return null;
        },
 ...
 */
function createForbiddenPatternsRule() {
    return {
        ...
        evaluate(request) {
            ...
            for (const { pattern, description } of FORBIDDEN_COMMAND_PATTERNS) {
                if (pattern.test(command)) {
                    return deny("command.forbidden_patterns", "FORBIDDEN_COMMAND", `Blocked: ${description} (pattern: ${pattern.source})`);
                }
            }
            return null;
        },
...
export function createCommandSafetyRules() {
    return [
