...
### 🔄 部署架构图 (v4.0 Conway 官方架构)

> ⚠️ **这是架构图的单一来源**
>
> 📖 **官方架构参考**: [ARCHITECTURE.md](ARCHITECTURE.md) - Conway 官方文档
...
│   ┌────────────────┐                              ┌────────────────────────┐   │
│   │ 📍 Layer 1     │                              │ 🌐 Layer 3             │   │
│   │ 本地开发(Mac)   │                              │ Conway Cloud           │   │
│   │                │                              │                        │   │
│   │ • VSCode       │      git push                │ • Sandbox VM (Linux)   │   │
│   │ • Claude Code  │─────────────────────────────▶│ • Receipt2CSV 服务      │   │
│   │ • Git          │                              │ • URL Metadata 服务     │   │
│   └───────┬────────┘                              │ • PM2 服务守护          │   │
│           │                                       │ • 持久化存储 (~/.auto/) │   │
│           ▼                                       └──────────▲─────────────┘   │
...
│   │ 🔀 Layer 2     │◀───────────────────────────────────────┘                 │
│   │ GitHub         │                                                            │
│   │ myfork/        │      Conway API (inference, domains)                      │
│   │ automaton      │──────────────────────────────────────────────────────────▶│
│   └────────────────┘                                              │             │
│                                                                   ▼             │
│                                                   ┌────────────────────────┐    │
...
| **Layer 3** | Conway Cloud | 🧠 **核心运行** | Sandbox VM + 推理 + 域名 + 持久化 |
| **Layer 4** | Base Mainnet | 链上身份 | ERC-8004 注册、USDC 收款 |

### Conway Cloud 核心能力

> 📖 **来源**: [scripts/conways-rules.txt](scripts/conways-rules.txt) - Conway 官方规则

- 🖥️ **Conway Cloud**: 完整 Linux VM，可部署服务、运行代码
- 🌐 **Conway Domains**: 域名购买和管理 (conway.domains)
- 🧠 **Conway Compute**: 推理付费 — Claude, GPT, Kimi 等
- 💾 **持久化存储**: `~/.automaton/` 目录持久化，重启不丢失
- ⛓️ **链上钱包**: USDC on Base，每次支付都是加密的、可编程的
...
│                                                         │
│  Step 2: 在 Conway Terminal 中更新配置                   │
...
> 💡 **注意**: 服务启动逻辑由 Conway 平台管理，通过 `boot_loader.mjs` 检测平台状态
