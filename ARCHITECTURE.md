...
- `PlanModeController` (a stricter, transition-validated phase machine in `plan-mode.ts`) appears to duplicate the `Orchestrator`'s own informal phase tracking, but is never imported by `orchestrator.ts` or `loop.ts` - it looks like dead code or an unfinished refactor target.
